...
class StudentRegisterSerializer1(serializers.ModelSerializer):
    class Meta:
        model = Student
        fields = ('id', 'email','first_name', 'last_name','sap_id','department', 'password')
        extra_kwargs = {'password': {'write_only': True}}

    def create(self, validated_data):
        user = Student.objects.create_user(email=validated_data['email'],sap_id=validated_data['sap_id'],password= validated_data['password'],department=validated_data['department'],is_student=True, is_teacher=False,  last_name= validated_data['last_name'],first_name= validated_data['first_name'])
...
class LibrarianRegisterSerializer1(serializers.ModelSerializer):
    class Meta:
        model = Librarian
        fields = ('id', 'email','librarian_id','joined_on','first_name', 'last_name', 'password')
        extra_kwargs = {'password': {'write_only': True}}

    def create(self, validated_data):
        user = Librarian.objects.create_user(email=validated_data['email'],password= validated_data['password'],is_student=False, is_teacher=False,is_librarian=True,librarian_id=validated_data['librarian_id'], last_name= validated_data['last_name'],first_name= validated_data['first_name'])
